...
void snapshot3d(Grid *g) {
  ...
  int tcm  = FRAMES*SizeX*SizeZ;
  int tcm2 = FRAMES*SizeX*SizeY;
  FILE *out;
  #pragma acc exit data copyout(pexMovie[0:tcm], pexMoviexy[0:tcm2])
  ...
  //     (Time - startTime) % temporalStride == 0) {
    ...
    printf("snapshoting \n");
    for (t =  0;  t < FRAMES; t++){
      // printf("t is %d, mid is %g \n", t, ExMovie(t, 5, 130));
      if (t >= startTime && 
      (t - startTime) % temporalStride == 0) {
        sprintf(filename, "ZXMovie/Ex%d.dat", frameX++);
        out = fopen(filename, "w");
        for (pp = 0; pp < SizeZ; pp++){
          for (mm = 0; mm < SizeX; mm++){ 
            temp = (float)ExMovie(t, mm, pp);
         	  fprintf(out, "%f  \n", temp);
          }
        }
    ...
    for (t =  0;  t < FRAMES; t++){
      ...
      if (t >= startTime && 
      (t - startTime) % temporalStride == 0) {
        sprintf(filename, "YXMovie/Ex%d.dat", frameX++);
        out = fopen(filename, "w");
        for (nn = 0; nn < SizeY; nn++){
          for (mm = 0; mm < SizeX; mm++){ 
            temp = (float)ExMovieXY(t, mm, nn);
            fprintf(out, "%f  \n", temp);
          }
        }
